"""
Augmentations module from visda-2017 winner
"""
import numpy as np
import cv2
import torch
    ...
    def augment(self, X):
        ...
        for i in range(len(X)):
            if X.shape[1] == 1:
                ...
                X[i, :, :, :] = cv2.warpAffine(X[i, :, :, :].transpose(1, 2, 0), xf_centred[i, :, :],
                                               (X.shape[3], X.shape[2])).transpose(2, 0, 1)
        ...
        if self.blur_range is not None and self.blur_range[0] is not None:
            ...
            for i in range(len(X)):
                ...
                if X.shape[1] == 1:
                    ...
                    X[i, :, :, :] = cv2.GaussianBlur(X[i, :, :, :].transpose(1, 2, 0), (ksize, ksize),
...
def get_aug_for_mnist():
    """Return augmentation for mnist dataset"""
    hflip = False
    xlat_range = 2
    affine_std = 0.1
