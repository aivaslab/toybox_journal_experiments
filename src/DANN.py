...
class Experiment:
    ...
    def train(self):
        ...
        runs_path = RUNS_DIR + self.source_dataset.upper() + "_" + self.target_dataset.upper() + "/exp_" \
                             + dt_now.strftime("%b-%d-%Y-%H-%M") + "/"
        tb_writer = tb.SummaryWriter(log_dir=runs_path)
        ...
        for ep in range(1, self.num_epochs + 1):
            ...
            for idx1, img1, labels1 in tqdm_bar:
                ...
                tb_writer.add_scalars(main_tag="Training", tag_scalar_dict={'CE Loss': ce_loss.item(),
                                      ...
                                      global_step=total_batches)
                tb_writer.add_scalars(main_tag="Training", tag_scalar_dict={'Avg CE Loss': ep_ce_loss / ep_batches,
                                                                            'Avg Dom Loss': ep_dom_loss / ep_batches,
                                                                            'Avg Total Loss': ep_tot_loss / ep_batches
                                                                            },
                                      global_step=total_batches)
