"""
Code for supervised experiments on SVHN
"""
...
class NNTrainer:
    ...
    def train(self):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for idxs, images, labels in tqdm_bar:
                ...
                if self.save and total_batches % self.save_frequency_batch == 0:
                    ...
                    network_file_name = OUTPUT_DIR + dt_now + "/svhn_network_batch_" + str(total_batches) + ".pt"
                    self.logger.info("Saving network to %s", network_file_name)
                    torch.save(self.net.network.state_dict(), network_file_name)
                    acc = self.eval_model(csv_file_name=OUTPUT_DIR + dt_now + "/svhn_eval_batch_" + str(total_batches)
                                                                            + ".csv")
            ...
            if self.save and epoch % self.save_frequency == 0:
                os.makedirs(OUTPUT_DIR + dt_now, exist_ok=True)
                network_file_name = OUTPUT_DIR + dt_now + "/svhn_network_epoch_" + str(epoch) + ".pt"
                self.logger.info("Saving network to %s", network_file_name)
                torch.save(self.net.network.state_dict(), network_file_name)
                acc = self.eval_model(csv_file_name=OUTPUT_DIR + dt_now + "/svhn_eval_epoch_" + str(epoch) + ".csv")
        ...
        if self.save:
            os.makedirs(OUTPUT_DIR + dt_now, exist_ok=True)
            network_file_name = OUTPUT_DIR + dt_now + "/svhn_network_final.pt"
            self.logger.info("Saving network to %s", network_file_name)
            torch.save(self.net.network.state_dict(), network_file_name)
            
            acc = self.eval_model(csv_file_name=OUTPUT_DIR + dt_now + "/svhn_eval_final.csv")
